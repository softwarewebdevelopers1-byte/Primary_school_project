    ...
    messages: [
      {
        ...
        content:
          [
            "You are an expert school timetable scheduling engine. Your sole job is to produce conflict-free, balanced, and realistic weekly lesson plans for a set of school classes.",
            "You must return ONLY a valid JSON object with no markdown, no explanation, and no preamble. Any non-JSON output is invalid.",
          ].join(" "),
      },
      {
        role: "user",
        content: [
          ...
          "ABSOLUTE CONSTRAINTS (violations will be rejected):",
          "1. TEACHER CONFLICT: A teacher cannot appear in two different classes during the same period on the same day. Cross-check every slot across all classes before outputting.",
          "2. SUBJECT COVERAGE: Every subject assigned to a class MUST appear in its weekly schedule. Do not invent subjects or omit assigned ones.",
          `3. SLOT COUNT: Each class must have EXACTLY ${subjectsPerDay} lesson entries per day, every day Monday through Friday. No more, no less.`,
          "4. SUBJECT IDS: Use only the exact subjectId strings provided in the class data. Never substitute names or create new IDs.",
          "QUALITY RULES (enforce strictly):",
          "5. EVEN DISTRIBUTION: Spread each subject as evenly as possible across the week. If a class has N subjects and S total slots, each subject should appear floor(S/N) or ceil(S/N) times, never more and never less.",
          "6. NO BACK-TO-BACK OVERLOAD: A subject must not appear in 3 or more consecutive periods for the same class on the same day.",
          "7. DAILY VARIETY: Each class should ideally have a different subject every period within a single day. Avoid scheduling the same subject more than ceil(weeklyCount / 5) + 1 times in one day.",
          "8. CROSS-CLASS DIVERSITY: Stagger the ordering of subjects across different classes to naturally reduce teacher conflicts across many simultaneous slots.",
          "9. ELECTIVE/SHARED SLOTS: If a subject has a non-null sharedSlotId, all subjects sharing that ID must be scheduled in the SAME slot on the SAME day because they run in parallel for different student groups. Their teachers must all be different.",
          "SCHEDULING STRATEGY:",
          "- Think of each day's slots as a grid: rows = classes, columns = time periods.",
          "- For each column/period, no teacherId may appear more than once across all rows.",
          "- Use a round-robin or offset rotation as a starting point, then adjust for conflicts.",
          "- Prioritize subjects with the fewest remaining slots to schedule first.",
          "- If a conflict is unavoidable in one day, redistribute to other days.",
